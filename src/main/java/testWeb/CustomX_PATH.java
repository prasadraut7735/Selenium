...
/**
 * No Rule No Restriction: Only the TagName should be written first Sequence or
 * count of the class does not matters if multiple classes comes under same
 * class
 * 
 * 
 * CSS Selector is a locator 1. ID == #id ex. #input-email tagName#id
 * input#input-email
 * 
 * 2. CLASS== .classname .form-control tagName.classname input.form-control
 ...
 * 3. ID and CLASS
 * 
 * #id.classname #username.login-email .classname#id .login-email#username
 * 
 * 4. TagName.CLASSNAME#ID TagName#ID.CLASSNAME TagName.C1#ID.C3,C2
 * 
 * 5. For AnyAttribute in CSS Selector
 * 
 * TagName[attr='value'] TagName[attr='value']#ID.CLASSNAME
 * TagName[attr='value'].CLASSNAME#ID [attr='value']
 * [attr1='value'][attr2='value'] #ID[attr='value'] .CLASSNAME[attr='value']
 * .CLASSNAME[attr='value']#ID #ID[attr='value'].CLASSNAME
 * 
 * 
 ...
 * 10. Parent and Child
 * 
 * parentTagName ChildTagName === Direct + Indirect* child parentTagName >
 * ChildTagName === Direct child
 *
 *
 * 11. Child to Parent: NA Backward Traversing is not applicable
 *
 * 12. Sibling in CSS: Only following-sibling is allowed TagName + Following
 * TagName label[for='input-email'] + input ===immediate following sibling
 ...
 * select#Form_getForm_Country > option: nth-child(n+4) ==start the counting
 * from 4 (Print value after 4) 
 * select#Form_getForm_Country > option:nth-child(4n) ==4*8*12 (Printing by multiplication)
 *
 *
 *
 * ## xpath ODD/EVEN //select[@id='Form_getForm_Country']/option[position() mod
 * 2=1] ==Odd indexing value printing
 * //select[@id='Form_getForm_Country']/option[position() mod 2=0] ==Even
 * indexing value printing
 *
 *
 * 14. Comma in CSS: Used for the elements present on the page or not with FE's
 * input#username, input#password, button#loginBtn
 *
 * 15. not in CSS: input.form-control:not(input[name='search'])
